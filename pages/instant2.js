...
function Instant({ number = 0, total = 0 }) {
  return (
    <FullScreenResizer listen={number}>
      <div className="present">有用的回應目前覆蓋 {total} 篇文章</div>
      <div className="verb">增加覆蓋</div>
      <div className="number">{number}</div>
...
const GET_ARTICLE_COUNT = gql`
  query GetArticleCount {
    ListArticles(filter: { hasArticleReplyWithMorePositiveFeedback: true }) {
      totalCount
    }
  }
`;
...
function InstantPage() {
  ...
  return (
    <div>
      <Head>
        <title>有用回應多覆蓋了 {number} 篇文章 - cofacts</title>
        <style jsx global>{`
          html {
